...
<div align="center">
<a href="viewMemberBalances.php">Back to Member Balances</a><br><br>
</div><br/>
...
<?php
	...
	$currentMemID = $_POST[memberID];
   	...
   	$query = $db->prepare("SELECT firstName, lastName FROM Member WHERE memberID=:currentMemID");
	...
	if($num_results!=0){   	
	    ...
	    echo "<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td><hr/></td></tr>";
	   	$query2 = $db->prepare("SELECT SUM(unitCost*quantity) AS debit FROM MemberLineItem INNER JOIN LineItem ON MemberLineItem.lineItemID = LineItem.lineItemID WHERE MemberLineItem.memberID=:currentMemID");
		$query2->execute(array('currentMemID'=>$currentMemID));
			$query2->setFetchMode(PDO::FETCH_ASSOC);
		$row2 = $query2->fetch();
		$tempDebitTotal=$row2[debit];

	   	$query3 = $db->prepare("SELECT SUM(amount) AS credit FROM Payment WHERE memberID=:currentMemID");
		$query3->execute(array('currentMemID'=>$currentMemID));
		...
		echo "<tr><td colspan=\"6\">There are currently no line items associated with this member.</td></tr>";
	}
	echo "</table>";
?>
